...
@export var rool_min_size: int = 3
...
func generate_dungeon(player: Entity) -> MapData:
	...
	for _try_room in max_rooms:
		var room_width: int = _rng.randi_range(rool_min_size, room_max_size)
		var room_height: int = _rng.randi_range(rool_min_size, room_max_size)
		...
		var has_intersections: bool = false 
